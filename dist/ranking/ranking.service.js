...
class RankingService {
    constructor(rmCalcService = new rm_calc_service_1.RmCalcService()) {
        this.rmCalcService = rmCalcService;
    }
    calculateRating(input, sessionCount = 0) {
        ...
        const dots = (0, dots_1.calculateDots)(rmKg, input.bodyweightKg, input.sex);
    ...
    validateInput(input) {
        if (!input) {
            throw new errors_1.BadRequestError('rating input is required');
        }
        if (!Number.isFinite(input.weight) || !Number.isFinite(input.reps) || !Number.isFinite(input.bodyweightKg)) {
            throw new errors_1.BadRequestError('weight, reps, and bodyweight must be numeric');
        }
        if (input.weight <= 0 || input.reps <= 0 || input.bodyweightKg <= 0) {
            throw new errors_1.BadRequestError('weight, reps, and bodyweight must be positive');
        }
        if (input.reps > 12) {
            throw new errors_1.BadRequestError('unreliable estimates, log a heavier set');
        }
        if (!['male', 'female'].includes(input.sex)) {
            throw new errors_1.BadRequestError('sex must be male or female');
        }
        if (!['squat', 'bench press', 'deadlift'].includes(input.exercise)) {
            throw new errors_1.BadRequestError('unsupported exercise');
        }
    }
    getPercentile(dots, exercise, sex) {
    ...
    getPopulationDistribution(exercise, sex) {
        const base = {
    ...
    getRankData(score) {
        const thresholds = [
            { rank: 'FRANK-Initiate', boundary: 2499, step: 1 },
            { rank: 'FRANK-Capable', boundary: 4999, step: 2 },
            { rank: 'FRANK-Solid', boundary: 6999, step: 3 },
            { rank: 'FRANK-Formidable', boundary: 8499, step: 4 },
            { rank: 'FRANK-Elite', boundary: 9499, step: 5 },
            { rank: 'FRANK-Untouchable', boundary: 10000, step: 6 },
        ];
    ...
    getWorldRecordDots(exercise, sex) {
    ...
    getLiftFrankScore(dots, worldRecordDots) {
